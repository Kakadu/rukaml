...
module CallArityAnal (CoCallGraph : sig
  ...
  let anal (_, _, p) bnd_unsafities =
    ...
    let take_safety_into_acc unsafety v_id b_co_calls v_arity =
      ...
      | _ -> v_arity
    in
    let merge_bnd_subexprs bnd_lam_ars bnd_lam_co_calls neigh = function
      | (rhs_co_calls, rhs_ars, ress), rhs_fv ->
        let p_ars = ar_union bnd_lam_ars rhs_ars in
        let p_co_calls =
          union bnd_lam_co_calls @@ union rhs_co_calls @@ cartesian rhs_fv neigh
        in
        p_co_calls, p_ars, ress
    in
    let rec anal_p conts ress inc_ar p int =
      ...
      | Call ((Lam (pat, c, lam_b) as f), a, CVar { id; _ }) ->
        (match find id conts with
         | `VarPatCont fin_anal ->
           let had_upd, unsafety = true, find_opt id bnd_unsafities in
           fin_lam_call_bnd_anal ~had_upd c lam_b int pat a unsafety (ress, fin_anal)
      ...
      | Letc ({ id = jv_id; _ }, Cont (CPVar { id; _ }, body), p) ->
        ...
        let conts2 = add jv_id (`VarPatCont fin_anal) conts in
        anal_p conts2 ress2 0 p int
      | Letc ({ id = jp_id; _ }, Cont (CPTuple _, body), p) ->
        let (b_co_calls, b_ars, ress2), b_fv =
          anal_p conts ress inc_ar body int |> ret_with_fv
        in
        let conts2 = add jp_id (`TuplePatCont (b_co_calls, b_ars, b_fv)) conts in
        anal_p conts2 ress2 0 p int
      | Letc ({ id = jp_id1; _ }, CVar { id = jp_id2; _ }, p) ->
        ...
        let conts2, ress2 =
          ...
          | `VarPatCont _ as c -> add jp_id1 c conts, upd_ress ()
          | `TuplePatCont _ as c -> add jp_id1 c conts, ress
    ...
    and anal_bnd1 ?(jv_specif = None) id conts body int ress inc_ar =
      ...
      let default v_arity =
        ...
        ( ress2
        , fun _ non_dead_hndl ->
            merge_bnd_subexprs bnd_lam_ars bnd_lam_co_calls neigh
  ...
  let pot_unsafe_bnds (_, _, p) =
    ...
    let rec check_p s_ars p n =
      ...
      | Letc ({ id = id2; _ }, CVar { id; _ }, b) ->
        let* bnd_unsafeties, conts = get in
        let conts =
          match find id conts with
          | cont -> add id2 cont conts
  ...
  type env_elem =
    ...
    | `ExTArgsCall of env_elem * ex_triv * ex_triv list
    ]
  ...
  let simpl (rec_flag, pat, p) (dead_vars, counts, barriers, fin_call_ars) =
    ...
    let rec simpl_p ?(cont_hndl = Def) etas p (env : env_elem IMap.t) =
      ...
      let let_or_ret_bnd pat b t fin_constr =
        ...
        let t = expand env v_ar prep_t in
      ...
      let ret_or_prep_call ?(etas = etas) call_ar prep_t c get_c' =
        ...
        | Future n ->
          ...
          if diff <= 0
          then Ret (get_c' (), expand env n prep_t)
          else (
      ...
      let call t1 t2 c get_c' =
        ...
        | prep_t1, (#ex_triv as prep_t2) ->
          ret_or_prep_call call_ar (`ExTArgsCall (prep_t1, prep_t2, [])) c get_c'
        | _, #ex_call ->
      ...
      let call_bnd ?(cont_hndl = cont_hndl) ?(extra_args = []) t1 t2 pat b c =
        ...
        | _, _, (#ex_triv as t2_prep) ->
          not_inl_ex_call @@ `ExTArgsCall (t1_prep, t2_prep, extra_args)
        | _, _, #ex_call ->
          failwith "unreachable: call shouldn't be inlined if its call arity is 0"
      in
      ...
      | _, Let (Recursive, pat, t, b) ->
        let t = expand env (v_ar pat) @@ prep_t_for_env env t in
        MACPS.Let (Recursive, translate_pat pat, t, simpl_p_sh b env)
      | _, CIf (t, p1, p2) -> CIf (simpl_t_sh t, simpl_p_sh p1 env, simpl_p_sh p2 env)
      | _, Primop (pat, i, t1, tt, p) ->
        let t1, tt = simpl_t_sh t1, List.map simpl_t_sh tt in
        Primop (translate_pat pat, i, t1, tt, simpl_p_sh p env)
      ...
      | Sub (Cont (pat, b1), cont_hndl, _), Letc (i, _, b2) ->
        ...
        in
        Letc (i, Cont (translate_pat pat, b1), simpl_p ~cont_hndl etas2 b2 env)
      ...
      | Sub ((Cont (pat, b) as c), cont_hndl, e_arg :: e_args), Ret (_, t) ->
        ...
        let ec =
          ...
          | prep_t1, _ -> `ExTArgsCall (prep_t1, e_arg, e_args)
        in
    ...
    and ex_call ?(cont_stuff = Present [], Def) c get_c' env =
      let rec helper args =
        ...
        in
        function
        | `ExTArgsCall ((#ex_call as ec), prep_t, prep_tt) ->
        ...
        | `ExTArgsCall ((#ex_triv as prep_t1), prep_t2, prep_tt) ->
          triv_triv_call prep_t1 prep_t2 prep_tt
      in
      helper
    and translate_prep_t_tuple env prep_t1 prep_t2 prep_tt =
      let trans = get_triv env in
      trans prep_t1, trans prep_t2, List.map trans prep_tt
    and ex_lam_call ?(cont_stuff = Present [], Def) pat i b prep_t args c get_c' env =
      ...
      let k_not_fully_inl () =
        let lam =
          MACPS.Lam ((translate_pat pat, pats), i, simpl_p (Present args) b env)
        in
        let t =
          match pat with
          | CPVar { id; _ } when ISet.mem id dead_vars -> MACPS.TUnit
          | _ -> expand env (v_ar pat) prep_t
        in
    ...
    and expand env v_ar =
      let expand_lam pat i b etas_count =
        ...
        MACPS.Lam ((translate_pat pat, eta_pats), i, simpl_p (Present eta_args) b env)
      in
      function
  ...
  let call_arity_anal cps_prog =
    pot_unsafe_bnds cps_prog |> anal cps_prog |> down_anal cps_prog |> simpl cps_prog
  ;;

  let call_arity_anal_debug cps_prog =
    let bnd_unsafeties = pot_unsafe_bnds cps_prog in
