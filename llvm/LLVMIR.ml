...
let debug_log_action action meth name =
  "[" ^ meth ^ ":" ^ action ^ " {" ^ name ^ "}]" |> debug_log
;;
...
let transform_if_needed (v : llvalue) (t : lltype) =
  if type_of v = t
  then v
  else if type_of v = int_type && t = pointer_type
  then LL.build_inttoptr v t
  else if type_of v = unit_type && t = pointer_type
  then LL.build_inttoptr v t
  else if type_of v = bool_type && t = pointer_type
  then LL.build_inttoptr v t
  else if type_of v = Llvm.pointer_type t
          && t <> pointer_type
          && type_of v <> pointer_type
  then LL.build_load v
...
module Builtins = struct
  ...
  let build_tuple_get name index size =
    let function_type =
      Llvm.function_type pointer_type [| Llvm.pointer_type (tuple_type size) |]
    in
    ...
    let ret_val =
      Llvm.build_extractelement tuple (LL.const_int int_type index) "" builder
    in
    LL.build_ret ret_val |> ignore
  ;;

  let declare_builtins () =
    LL.declare_function "print_int" (Llvm.function_type unit_type [| int_type |])
    |> ignore;
    LL.declare_function "print_bool" (Llvm.function_type unit_type [| bool_type |])
    |> ignore;
    LL.declare_function "trace_int" (Llvm.function_type int_type [| int_type |]) |> ignore;
    LL.declare_function "trace_bool" (Llvm.function_type bool_type [| bool_type |])
    |> ignore;
    LL.declare_function "get_int_arg" (Llvm.function_type int_type [| int_type |])
    |> ignore;
    LL.declare_function
      "applyN"
      (Llvm.var_arg_function_type pointer_type [| pointer_type; int_type |])
    |> ignore;
    LL.declare_function
      "alloc_closure"
      (Llvm.function_type pointer_type [| pointer_type; int_type |])
    |> ignore;
    LL.declare_function "gc_alloc" (Llvm.function_type pointer_type [| int_type |])
    |> ignore;
...
module LlvmFunction = struct
  let rec to_llvm_type = function
    ...
    | Typedtree.Arrow _ as arr ->
      let args, ret_type = TypedtreeHelper.fold_arrow arr in
      Llvm.function_type
        (to_llvm_type ret_type)
        (List.map to_llvm_type args |> Array.of_list)
      |> Llvm.pointer_type
  ...
  let build_function name args body codegen =
    let ret_val_builder _ = codegen body in
  ...
  let build_init_globals codegen =
    let ret_val_builder _ =
      let globs = Hashtbl.to_seq global_initializers in
      let init_globs =
        globs
        |> Seq.map (function name, init ->
  ...
  let build_direct_call f args =
    let prepare_arg v t = transform_if_needed (alloc_closure_if_needed v) t in
    let real_args =
  ...
  let build_partial_application f args expected_args_count real_args_count =
    ...
    let final_args =
      callee
      :: LL.const_int int_type real_args_count
      :: List.map alloc_closure_if_needed args
    in
  ...
  let build_closure_call f args =
    ...
    let final_args =
      callee_f
      :: LL.const_int int_type arg_number
      :: List.map alloc_closure_if_needed args
    in
    LL.build_call applyN final_args |> Result.ok
  ;;
  ...
  let build_call name args codegen =
    ...
    if Builtins.is_builtin_binop name
    then (
      match args with
      ...
      | None ->
        ...
        (match expected_args_count - real_args_count with
         ...
         | _ ->
           build_partial_application calling_f args expected_args_count real_args_count)
      ...
      List.fold_left
        (fun vec v ->
          ...
          build_insertelement
            vec
            v
            (LL.const_int (i32_type context) i.contents)
            ""
            builder)
        (LL.build_load array)
        values
...
let rec codegen = function
  ...
  | Typedtree.TTuple (fst, snd, other, _) ->
    LlvmTuple.create_tuple (fst :: snd :: other) codegen
;;

let codegen_top_level = function
  ...
  | Typedtree.TLet (_, name, _, value_expr, in_expr) ->
    let global_value =
      Llvm.define_global name (Llvm.const_null pointer_type) the_module
    in
...
let value_binding_to_expr (x : Typedtree.value_binding) =
  ...
  Typedtree.TLet
    ...
    , x.tvb_typ
    , x.tvb_body
    , Typedtree.TVar (name, Typedtree.type_of_expr x.tvb_body) )
  |> Typedtree.compact_expr
;;

let generate_llvm
  (value_bindings : Typedtree.value_binding list)
  emit_llvm
  llvm_out
  object_out
  =
  ...
  let result =
    let* _ =
      List.map (codegen_top_level << value_binding_to_expr) value_bindings
      |> list_or_error
    in
