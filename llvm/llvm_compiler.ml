...
module ToLLVM = struct
  ...
  let run cfg =
    ...
    let promote_error r =
      Result.map_error ~f:(fun x -> (x :> [ Parsing.error | Inferencer.error | LLVMIR.error])) r
    in

    let* value_bindings = get_value_bindings text in
    LLVMIR.generate_llvm value_bindings cfg.emit_llvm cfg.llvm_out cfg.object_out |> promote_error
  ;;
end
...
let print_errors = function
  ...
  | #Miniml.Inferencer.error as e -> Format.printf "Inferencer error: %a\n%!" Inferencer.pp_error e
...
let () =
  Stdlib.Arg.parse
    [ 
      ("-emit-llvm", Stdlib.Arg.String (fun x -> cfg.llvm_out <- x; cfg.emit_llvm <- true), "" );
