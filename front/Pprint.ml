...
(** TODO? distinguish [ let f (x, y) = ... ] and [ let x, y = ... ] *)
type pp_pattern_ctx =
...
let rec pp_expr_helper ?(ps = true) ppf = function
  ...
  | ELam (pat, e) ->
    fprintf ppf "@[(fun %a ->@[ %a@])" (pp_pattern CtxLeftSideFun) pat no_pars e
  ...
  | EMatch (e, (pe, pes)) ->
    let pp_cases ppf () =
      let pp_case ppf (p, e) =
        fprintf ppf "@[| %a -> %a@]" (pp_pattern CtxLeftSideMatch) p maybe_pars e
      in
